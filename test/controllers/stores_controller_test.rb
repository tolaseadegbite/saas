...
class StoresControllerTest < ActionDispatch::IntegrationTest
  ...
  test "redirect create when not logged in" do
    post business_stores_path(@business), params: { product: { name: '' } }
    assert_redirected_to new_user_session_url
    assert_not flash.empty?
  end
  ...
  test "redirect update when not logged in" do
    patch business_product_path(@business, @store), params: { business: { name: '' } }
    assert_redirected_to new_user_session_url
    assert_not flash.empty?
  end

  test "redirect destroy when not logged in" do
    delete business_product_path(@business, @store)
    assert_redirected_to new_user_session_url
    assert_not flash.empty?
  end
  ...
  test "redirect update when logged in as wrong user" do
    sign_in @other_user
    name = 'new name'
    patch business_store_path(@business, @store), params: { business: { name: name } }
    assert_redirected_to businesses_url
    assert_not flash.empty?
  end
